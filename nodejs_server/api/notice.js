...
router.get('/', (req, res) => {
  const sql = 'SELECT * FROM noticeList ORDER BY date DESC limit 1';
...
router.get('/all', (req, res) => {
  const sql = 'SELECT * FROM noticeList ORDER BY date desc';
...
router.get('/list/:page', (req, res) => {
  const sql = `SELECT * FROM noticeList ORDER BY date DESC LIMIT 20 OFFSET ${(req.params.page
    - 1)
    * 20}`;
